...
function Bottoms() {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/api/bottoms')
  ...
  const handlePlaceBid = async (itemId) => {
    ...
    try {
      ...
      formData.append('itemType', 'bottom')
      ...
      <div className="container">
        ...
        {items.length === 0 ? (
          <div className="no-items">
            <p>No bottoms available at the moment.</p>
          </div>
        ) : (
                ...
                <div key={item.id} className={`item-card ${auctionClosed ? 'auction-closed' : ''}`}>
                  ...
                  <div className="item-body">
                    <div className="item-title">
                      {item.description || `Bottom #${item.id}`}
                    </div>
                    ...
                    <div className="measurements">
                      ...
                      <p>Waist: {item.waistLength} | Inseam: {item.inseamLength} | Outseam: {item.outseamLength}</p>
                      <p>Hip: {item.hipLength} | Rise: {item.riseLength}</p>
                    </div>
                  ...
                  <div className="item-footer">
                    ...
                    <button
                      ...
                      onClick={() => navigate(`/bid-history?itemType=bottom&itemId=${item.id}`)}
                    >
                      Bid History
                    </button>
                  </div>
        ...
        {selectedItem && (
          <div className="bid-modal-overlay" onClick={() => setSelectedItem(null)}>
            <div className="bid-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <h2>Place Bid on {selectedItem.description || `Bottom #${selectedItem.id}`}</h2>
